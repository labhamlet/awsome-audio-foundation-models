"""M2D-CLAP Pre-training Script for Stage 1
...
class CapEmbSpecDataset(torch.utils.data.Dataset):
    def __init__(self, base_folder, files_main, files_bg_noise, file_caption, crop_size, noise_ratio=0.0,
        ...
        self.bg_index = []
        # find caption files fron folder
        if Path(file_caption).is_dir():
            file_caption = ','.join(sorted([str(f) for f in Path(file_caption).glob('*.npy')]))
        # load captions
        self.capembs = []
    ...
    def get_random_caption(self, ytid, folder_stem):
        ...
        if len(emb_list) == 0:
            for capemb in self.capembs:
                if folder_stem in capemb:
                    cur = capemb[folder_stem]
                    cur = [cur] if len(cur.shape) == 1 else [cur[i] for i in range(cur.shape[0])]  # prepare list of captions
                    emb_list.extend(cur)
        if not emb_list:
            print('MISSING', ytid, folder_stem)
    ...
    def __getitem__(self, index, fixed_noise=False):
        ...
        }[filepath[-2]] if filepath[0] == 'wavcaps_lms' else ''  # folder name conversion for wavcaps_lms
        if filepath[0] == 'clotho_lms':
...
def get_args_parser():
    parser = argparse.ArgumentParser('M2D-CLAP pre-training', add_help=False)
    ...
    parser.add_argument('--sr', default='16k', type=str, metavar='SR', help='Sampling rate of the input audio.')  # just for adding to the output folder name
    parser.add_argument('--text_encoder', default='B', type=str, metavar='SR', help='Text encoder type: A=GTE base, B=BERT base, N=NVEmbedV2.')
    ...
    parser.add_argument('--loss_m2d_end', default=1., type=float, help='Loss of M2D masked prediction')
    parser.add_argument('--loss_off', default=.01, type=float, help='Loss of offline target')
    parser.add_argument('--loss_off_end', default=.01, type=float, help='Offline target loss at the end of training')
    ...
    parser.add_argument('--sem_mode', default=1, type=int,
                        help='CLAP semantic mode (audio projector blocks).')
    ...
    parser.add_argument('--load', default='', help='load from a checkpoint regardless of model name')
...
def load_model(args, model_without_ddp, optimizer, loss_scaler, delta_epoch=1, strict=True):
    ...
    if args.load:
        weights = str(args.load).split(',')
        checkpoint = torch.load(weights[0], map_location='cpu')['model']
        msg = model_without_ddp.load_state_dict(checkpoint, strict=False)
        print(f'Load encoder/decoder weights from {weights[0]} -- {msg}')

        if len(weights) > 1:
            checkpoint = torch.load(weights[1], map_location='cpu')['model']
            proj_weights =  {k.replace('backbone.', ''): v for k, v in checkpoint.items() if k.startswith('backbone.audio_proj.')}
            proj_weights |= {k.replace('backbone.', ''): v for k, v in checkpoint.items() if k.startswith('backbone.text_proj.')}
            msg = model_without_ddp.load_state_dict(proj_weights, strict=False)
            print(f'Load projector weights {weights[1]} -- loaded keys: {list(proj_weights.keys())}, unexpected keys: {msg.unexpected_keys}')
...
def main(args):
    ...
    model = models_mae.__dict__[args.model](img_size=args.input_size, patch_size=args.patch_size, decoder_depth=args.decoder_depth,
        ...
        off_emb_dim=dataset_train.caption_dim, clip_args=[global_rank, num_tasks], sem_mode=args.sem_mode)
    ...
    last_subprocess, cur_loss_m2d, cur_loss_off = None, args.loss_m2d, args.loss_off
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # loss lambda linear interpolation
        if args.loss_m2d != args.loss_m2d_end:
            cur_loss_m2d = (epoch1 * (args.loss_m2d_end - args.loss_m2d))/args.epochs + args.loss_m2d
            model_without_ddp.loss_m2d = cur_loss_m2d
        if args.loss_off != args.loss_off_end:
            cur_loss_off = (epoch1 * (args.loss_off_end - args.loss_off))/args.epochs + args.loss_off
            model_without_ddp.loss_off = cur_loss_off
...
arg_conf_defaults = {
    ...
    'accum_iter': (1, 'a', 'asis'),
    'loss_m2d': (1.0, 'lm', 'z'),
    'loss_m2d_end': (1.0, 'lme', 'z'),
    'loss_off': (0.01, 'lo', 'z'),
    'loss_off_end': (0.01, 'loe', 'z'),
...
def complete_args():
    ...
    if not args.output_dir:
        args.output_dir = f'{args.model}-{_input_size}p{_patch_size}p{args.sr}p{args.text_encoder}'

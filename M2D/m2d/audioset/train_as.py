"""Masked Modeling Duo (M2D) Pre-training Script for M2D-AS
...
class LabelEmbSpecDataset(torch.utils.data.Dataset):
    def __init__(self, base_folder, files_main, files_bg_noise, labels, crop_size, noise_ratio=0.0,
        ...
        self.ds1.norm_stats = (self.norm_stats[0], 1.0)
        ...
        self.labels = torch.tensor(oh_enc.fit_transform([str(ls).split(',') for ls in labels]), dtype=torch.float32)
        self.classes = oh_enc.classes_
        self.one_hot_dim = self.labels.shape[-1]
        print('Labels:', len(self.labels), ' classes:', self.one_hot_dim)
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='m2d_as_vit_base', type=str, metavar='MODEL', help='Name of model to train')
    ...
    parser.add_argument('--loss_off', default=.1, type=float, help='Loss of offline target')
    ...
    parser.add_argument('--data_path', default='data', type=str,
                        help='dataset path')
    parser.add_argument('--csv_main', default='data/files_as_weighted.csv', type=str,
...
def main(args):
    ...
    dataset_train, dataset_val = build_as_clf_dataset(args)
    ...
    model = models_mae.__dict__[args.model](img_size=args.input_size, patch_size=args.patch_size, decoder_depth=args.decoder_depth,
        ...
        off_emb_dim=dataset_train.one_hot_dim, norm_stats=dataset_train.norm_stats)
...
arg_conf_defaults = {
    'csv_main': ('data/files_as_weighted.csv', 'M', 'path'),
    'csv_bg_noise': ('', 'D', 'path'),

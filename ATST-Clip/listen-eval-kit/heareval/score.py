...
class SourceLocalX(ScoreFunction):
    ...
    def _compute(self, predictions: np.ndarray, targets: np.ndarray, **kwargs) -> float:
        ...
        try:
            ...
            source_localization_error = np.abs(np.subtract(predictions[...,0], targets[...,0])).mean()
...
class SourceLocalY(ScoreFunction):
    ...
    def _compute(self, predictions: np.ndarray, targets: np.ndarray, **kwargs) -> float:
        ...
        try:
            # Mean source localization 
            source_localization_error = np.abs(np.subtract(predictions[...,1], targets[...,1])).mean()
            return float(source_localization_error)
        except ValueError:
            return np.nan
...
class SourceLocalZ(ScoreFunction):
    ...
    def _compute(self, predictions: np.ndarray, targets: np.ndarray, **kwargs) -> float:
        ...
        try:
            # Mean source localization 
            source_localization_error = np.abs(np.subtract(predictions[...,2], targets[...,2])).mean()
            return float(source_localization_error)
        except ValueError:
            return np.nan
...
class DOE(ScoreFunction):
    ...
    def _compute(self, predictions: np.ndarray, targets: np.ndarray, **kwargs) -> float:
        ...
        try:
            # Mean source localization 
            pred_az_rad, pred_el_rad = cartesian_to_spherical(predictions[...,0], 
                                                                  predictions[...,1], 
                                                                  predictions[...,2])
            target_az_rad, target_el_rad = cartesian_to_spherical(targets[...,0], 
                                                                  targets[...,1], 
                                                                  targets[...,2])
            ...
            cos_dist = (np.sin(target_el_rad) * np.sin(pred_el_rad) + 
                        np.cos(target_el_rad) * np.cos(pred_el_rad) * 
                        np.cos(target_az_rad - pred_az_rad))
...
class DOA(ScoreFunction):
    ...
    def _compute(self, predictions: np.ndarray, targets: np.ndarray, **kwargs) -> float:
        ...
        try:
            ...
            source_localization_error = 2 * np.arcsin(sqrt_pred_error / 2) * (180 / np.pi)
...
class MeanAngularError(ScoreFunction):
    ...
    def _compute(self, predictions: np.ndarray, targets: np.ndarray, **kwargs) -> float:
        ...
        try:
            ...
            targets_normalized = targets / np.linalg.norm(targets, axis=1, keepdims=True)
            predictions_normalized = predictions / np.linalg.norm(predictions, axis=1, keepdims=True)

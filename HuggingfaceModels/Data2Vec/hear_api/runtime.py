...
class RuntimeData2Vec(torch.nn.Module):
    def __init__(self, 
                 model_size, 
                 **kwargs) -> None:
        super().__init__()
        ...
        if model_size == "base":
            self.model = AutoModel.from_pretrained("facebook/data2vec-audio-base")
            ...
            self.model = AutoModel.from_pretrained("facebook/data2vec-audio-large")
            # sample rate and embedding sizes are required model attributes for the HEAR API
            self.embedding_size = 1024
            self.scene_embedding_size = self.embedding_size
            self.timestamp_embedding_size = self.embedding_size
            self.extractor = AutoFeatureExtractor.from_pretrained("facebook/wav2vec2-large")

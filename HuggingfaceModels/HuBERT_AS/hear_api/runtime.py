...
class RuntimeHuBERTAS(torch.nn.Module):
    def __init__(self, 
        ...
        if model_size == "base":
            self.model = AutoModel.from_pretrained("ALM/hubert-base-audioset")
            # sample rate and embedding sizes are required model attributes for the HEAR API
            self.embedding_size = 768
            self.scene_embedding_size = self.embedding_size
            self.timestamp_embedding_size = self.embedding_size
            self.extractor = AutoFeatureExtractor.from_pretrained("ALM/hubert-base-audioset")

        elif model_size == "large":
            self.model = AutoModel.from_pretrained("ALM/hubert-large-audioset")
            # sample rate and embedding sizes are required model attributes for the HEAR API
            self.embedding_size = 1024
            self.scene_embedding_size = self.embedding_size
            self.timestamp_embedding_size = self.embedding_size
            self.extractor = AutoFeatureExtractor.from_pretrained("ALM/hubert-large-audioset")

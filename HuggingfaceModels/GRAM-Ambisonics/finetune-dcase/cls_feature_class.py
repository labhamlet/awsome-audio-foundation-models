...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        self.extract =  AutoFeatureExtractor.from_pretrained("labhamlet/gramt-ambisonics", trust_remote_code=True)
    ...
    def _get_foa_intensity_vectors(self, linear_spectra):
        ...
        I = 2 * np.real(np.conj(W) * XYZ)
        
        # Compute energy with epsilon for numerical stability
        # E = eps + |W|^2 + |X|^2 + |Y|^2 + |Z|^2
        W_power = np.squeeze(np.abs(W) ** 2)
        xyz_power = np.sum(np.abs(XYZ) ** 2, axis=-1)
        E = self.eps + W_power + xyz_power
        
        # Normalize intensity vectors
        I_norm = I / np.expand_dims(E, axis = -1)
        foa_iv = np.transpose(np.dot(np.transpose(I_norm, (0,2,1)), self._mel_wts), (0,2,1))

...
class SpatialAST(_VisionTransformer):
    ...
    # overwrite original timm
    def doa(self, waveforms, mask_t_prob = 0, mask_f_prob = 0):
        B, C, T = waveforms.shape
        waveforms = waveforms.reshape(B * C, T)
        real, imag = self.spectrogram_extractor(waveforms) 
        log_mel = self.logmel_extractor(torch.sqrt(real**2 + imag**2)).reshape(B, C, -1, 128)
    ...
    def classify(self, waveforms, mask_t_prob = 0, mask_f_prob = 0):
        B, C, T = waveforms.shape
        ten_seconds_samples = 32000 * 10
        if T <= ten_seconds_samples:
            waveforms = waveforms.reshape(B * C, T)
            real, imag = self.spectrogram_extractor(waveforms) 
            log_mel = self.logmel_extractor(torch.sqrt(real**2 + imag**2)).reshape(B, C, -1, 128)
            log_mel = self.bn(log_mel)
            ...
            x = self.forward_features_mask(x, mask_t_prob=mask_t_prob, mask_f_prob=mask_f_prob)
            cls_token = x[:, 0]
            cls_token = self.dis_norm(cls_token)
            return cls_token

        else:
            cls_tokens = []
            # Calculate padding needed to reach the next multiple of 10 seconds
            target_len = math.ceil(T / ten_seconds_samples) * ten_seconds_samples
            padding_needed = target_len - T
            # Pad the waveform at the end of the time dimension
            padded_waveforms = F.pad(waveforms, (0, padding_needed)) # Pads last dim (T)

            # Calculate the number of 10-second segments
            num_segments = target_len // ten_seconds_samples

            # Process each 10-second segment
            for i in range(num_segments):
                start = i * ten_seconds_samples
                end = start + ten_seconds_samples
                # Extract chunk: (B, C, ten_seconds_samples)
                current_chunk = padded_waveforms[:, :, start:end]

                # --- Apply the SAME processing steps as the short audio case to the chunk ---
                chunk_T = current_chunk.shape[2] # Should be ten_seconds_samples
                flat_chunk = current_chunk.reshape(B * C, chunk_T)

                real, imag = self.spectrogram_extractor(flat_chunk)
                log_mel = self.logmel_extractor(torch.sqrt(real**2 + imag**2)).reshape(B, C, -1, 128)
                log_mel = self.bn(log_mel)
                IPD = torch.atan2(imag[1::2], real[1::2]) - torch.atan2(imag[::2], real[::2])
                x = torch.cat([log_mel, torch.matmul(torch.cat([torch.cos(IPD), torch.sin(IPD)], dim=1), self.logmel_extractor.melW)], dim=1)
                ...
                cls_token = x[:, 0]
                cls_token = self.dis_norm(cls_token)
                cls_tokens.append(cls_token) # Add chunk's CLS token to the list

            # Stack tokens from all segments
            stacked_tokens = torch.stack(cls_tokens, dim=0)

            # Compute mean across the segment dimension (dim=0)
            mean_cls_token = torch.mean(stacked_tokens, dim=0) # Shape (B, EmbDim)
